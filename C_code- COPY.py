...
class Calc(Parser):
    ...
    tokens = (
        ...
        'LPAREN', 'RPAREN', 'SEMICOLON','COLON', 'COMMA', 'LBRACE', 'RBRACE', 'LBRACKET', 'RBRACKET', 'IF', 'ELSE', 'WHILE', 'PRINT',
        'LESSTHAN', 'GREATERTHAN', 'EQUALTO', 'READINT', 'OROR',
    )
    ...
    def p_expression_indexing(self, p):
        ...
        p[0] = AST("list_indexing", children=[p[1], p[3]])
    ...
    def p_expression_list_slice(self, p):
        """expression : expression LBRACKET opt_expression COLON opt_expression RBRACKET"""
        p[0] = AST("list_slice", children=[p[1], p[3], p[5]])
    ...
    def p_block_rule(self, p):
        ...
        p[0] = AST("BLOCK", children=[p[2]])
# figure out which is the actual expressor and which is the definition, potentially combine into one.
    ...
    def p_statement_while(self, p):
        ...
        p[0] = AST("WHILE", children=[p[3], p[5]])

    def p_statement_list(self, p):
        """statement_list : statement statement_list
                                | statement"""
        if len(p) == 3:
            p[0] = AST("LIST", children=[p[1], p[2]])
        else:
            p[0] = p[1]
    ...
    def p_expression_oror(self, p):
        """expression : expression OROR expression"""
        p[0] = AST("OROR", children=[p[1],p[3]])
    ...
    class emit_mips:
        ...
        def generate(self, node):
            if node is None:
                ...
                return
            if node.name == 'NUMBER':
                self.output.append(f"li $t0, {node.value}")
            elif node.name == 'STRING':
                self.output.append(f"# string {node.value}")
            elif node.name == 'PLUS':
                ...
                self.output.append("move $t1, $t0")
                self.generate(node.children[1])
                self.output.append("mul $t0, $t1, $t0")
            elif node.name == 'DIVIDE':
                self.generate(node.children[0])
                self.output.append("move $t1, $t0")
                self.generate(node.children[1])
                self.output.append("div $t1,$t0")
                self.output.append("mflo $t0")
                ...
                self.generate(node.children[0])  # condition
                self.output.append(f"beqz $t0, {else_label}")
                self.generate(node.children[1])  # then
                ...
                self.output.append(f"j {end_label}")
                self.output.append(f"{else_label}:")
                self.generate(node.children[2])  # else
                self.output.append(f"{end_label}:")
            elif node.name == 'READINT':
                self.output.append("li $v0, 5")
                self.output.append("syscall")
                self.output.append("move $t0, $v0")
            elif node.name == 'LESSTHAN':
                self.generate(node.children[0])
                self.output.append("move $t1, $t0")
                self.generate(node.children[1])
                self.output.append("slt $t0, $t1, $t0")
            elif node.name == 'GREATERTHAN':
                self.generate(node.children[0])
                self.output.append("move $t1, $t0")
                self.generate(node.children[1])
                self.output.append("slt $t0, $t0, $t1")
            elif node.name == 'EQUALTO':
                self.generate(node.children[0])
                self.output.append("move $t1, $t0")
                self.generate(node.children[1])
                self.output.append("seq $t0, $t1, $t0")  # set if equal
            else:
                print(f"Unknown node: {node.name}")
            # elif node.value == '<':
